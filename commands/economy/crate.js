exports.run = async (client, message, args) => {
	...
	if(args[0]) {
		...
		const kcrate = Object.keys(client.tools.crates[crate].items);
		const ecrate = Object.values(client.tools.crates[crate].items);
		let m = '**You found:\n\n';
		for(let i = 0; i < kcrate.length; i++) {
			if(kcrate[i] === 'Cash') {
				...
				const emote = client.items.Materials[kcrate[i]].emote;
				...
				const drops = Math.floor(Math.random() * ecrate[i][1]) + ecrate[i][0];
				const emote = client.items.Food[kcrate[i]].emote;
