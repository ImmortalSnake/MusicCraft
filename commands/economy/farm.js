exports.run = async (client, message, args, { mc, prefix }) => {
	...
	if(!hoe) return message.channel.send(`You do not have an hoe. Use \`${prefix}craft wooden hoe\` to get a hoe`);
	const ihoe = inventory.tools.find(x=>x.name === hoe.value);
	if(ihoe.value.durability < 1) return message.channel.send(`You cannot use this hoe anymore as it is broken, please use \`${prefix}repair ${hoe.value}\` to repair it`);
