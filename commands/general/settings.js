exports.run = async (client, message, args, { settings, prefix }) => {
	if(args[0]) {
		...
		case 'djrole': {
			...
			const role = message.mentions.roles.first() || message.guild.roles.find(r => r.name === args[1]);
			let toggle = false;
	...
	const embed = client.embed(message, { title: '**SETTINGS**' })
...
function genEmbed(client, message, settings, type) {
	const info = client.settings[type];
